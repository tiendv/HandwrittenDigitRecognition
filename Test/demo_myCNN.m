...
% Modified by  dattt,tiendv 
% Add more models  

train_set = lib.load_data.loadMNISTImages('data/train-images.idx3-ubyte')';
train_label = lib.load_data.loadMNISTLabels('data/train-labels.idx1-ubyte');
train_scale = size(train_set);
load('model/svmmodel.mat', 'svmmodel');
...
tmp             =   load('model/myLeNet5-example.mat');
...
methodButton = uibuttongroup('Visible', 'off', ...
                              'Parent', hMainFigure, ...
                              'Position', [0.8 0 0.2 2], ...
                              'SelectionChangedFcn',@methodSelection);
                          
r1 = uicontrol(methodButton,'Style',...
                  'radiobutton',...
                  'String','CNN',...
                  'Position',[5 350 100 30],...
                  'HandleVisibility','off');
              
                  'String','KNN',...
r2 = uicontrol(methodButton,'Style','radiobutton',...
                  'Position',[5 250 100 30],...
                  'HandleVisibility','off');
...
methodButton.Visible = 'on';
              
svmButton = uibuttongroup('Visible', 'off', ...
                          'Parent', hMainFigure, ...
                          'Position', [0.9 0 0.2 2], ...
                          'SelectionChangedFcn', @svmKernel);
                      
s1 = uicontrol(svmButton,'Style',...
                  'radiobutton',...
                  'String','Linear Kernel',...
                  'Position',[5 150 100 30],...
                  'HandleVisibility','off');
              
s2 = uicontrol(svmButton,'Style','radiobutton',...
                  'String','Poly 2 Kernel',...
                  'Position',[5 100 100 30],...
                  'HandleVisibility','off');
              
s3 = uicontrol(svmButton,'Style','radiobutton',...
                  'String','RBF Kernel',...
                  'Position',[5 50 100 30],...
                  'HandleVisibility','off');

kNNSelection   =   uicontrol(...
                    'Parent',hMainFigure,...
                    'Units','characters',...
                    'HorizontalAlignment','left',...
                    'Position',[120 17 10 1.62],...
                    'String','5',...
                    'Enable','inactive',...
                    'Visible', 'off', ...
                    'Style','edit', ...
...
function methodSelection(source,event)
       set(svmButton, 'Visible', 'off');
       set(kNNSelection, 'Visible', 'off');
       useMethod = event.NewValue.String;
       if (useMethod == 'KNN')
           set(kNNSelection, 'Visible', 'on');
       elseif (useMethod == 'SVM')
           set(svmButton, 'Visible', 'on');
       end
end

function svmKernel(source,event)
       tmp = event.NewValue.String;
       if (strcmp('Linear Kernel', tmp))
           svmMethod = 1;
       elseif (strcmp('Poly 2 Kernel', tmp))
           svmMethod = 2;
       elseif (strcmp('RBF Kernel', tmp))
           svmMethod = 3;
       end
end
...
if nargout>0
    ...
    function hRecognizeButtonCallback(hObject, eventdata)
        ...
        elseif (useMethod == 'KNN')
            Ip = preproc_kmg_knn(mIconCData)';
            Ip = reshape(imresize(Ip, [28 28]), 1, 28 * 28);
            dist = zeros(train_scale(1),1);
            for i = 1:train_scale(1)
                train_point = train_set(i, :);
                tmp = Ip - train_point;
                dist(i) = sqrt(sum(tmp.*tmp));
            end
            dist_tmp = sort(dist);
            num = zeros(10, 1);
            numK = str2num(get(kNNSelection, 'String'));
            for k=1:numK
            ...
            for out_i = 1:numel(num),
                set(hResultDigits(out_i), 'ForegroundColor', [1 1 1]*(1.8-num(out_i))/3.6)
            end
            maxIdx = maxIdx - 1;
            set(hResultText, 'string', char('0'+maxIdx), 'ForegroundColor', [1 1 1]*(1.8-maxNum)/3.6)
            
        elseif (useMethod == 'SVM')
            Ip = preproc_kmg_knn(mIconCData)';
            Ip = reshape(imresize(Ip, [28 28]), 1, 28 * 28);
            [predicted_label, ~, ~] = lib.libsvm.matlab.svmpredict([1], Ip, svmmodel{svmMethod});
            set(hResultText, 'string', char('0'+predicted_label(1)), 'ForegroundColor', [1 1 1]*(1.8-1.0)/3.6);
        end
...
function out = preproc_kmg_knn(id)
    %Preprocess single image
    Inorm = id(:,:,1);
    Inorm(~isfinite(Inorm)) = 1;
    out = abs(Inorm-1)';
